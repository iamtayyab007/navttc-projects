...
function printEvenOdd() {
  const numbers = ["apple", "banana", "grapes", "berry", "tacos"];
  const evenNumbers = [];
  const oddNumbers = [];
...
function printEvenOddAsyncTask() {
  const numbers = [
    "apple",
    "banana",
    "grapes",
  ...
  for (let i = 0; i < numbers.length; i++) {
    setTimeout(() => {
      ...
      document.getElementById("evenOutputAsyncTask").textContent =
        evenNumbers.join(", ");
      document.getElementById("oddOutputAsyncTask").textContent =
        oddNumbers.join(", ");
    }, i * 1000);
  }
}
